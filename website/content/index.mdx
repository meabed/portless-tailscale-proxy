...
No install, no config — it discovers your running dev servers and shares them all
through one Tailscale URL. First time? Run `npx tailscale-proxy doctor` to check your
setup, or follow the 4-step [Installation](/installation) guide.

```bash
brew install meabed/tap/tsp && tsp     # prefer a binary? install, then run `tsp`
```

<Callout type="info">
  **Prefer clicking to typing?** There's a native **menu-bar app** for macOS,
  Windows, and Linux — [download it here](/desktop).
</Callout>
